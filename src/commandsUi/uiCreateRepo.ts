  ...
  try {
    ...
    if (orgsUserCanCreateRepo.length === 0) {
      ...
      const pick = await myQuickPick({
        ...
        title:
          'Should the new repository be created in your account or in an Organization?',
      });
      ...
      if (!havePickedUser) organizationLogin = pick.description!;
    }
    ...
    if (!name)
      // Don't allow empty names. This also catches `undefined`, if user pressed Esc.
      return;
    ...
    if (preRepoCreationResult === 'cancel')
      // Quit if true
      return;

    const newRepository = await createGitHubRepository({
      name,
      description,
      isPrivate,
      organizationLogin,
      ...
      await Promise.all([
        ...
        (async () => {
          ...
          const answer = await vscode.window.showInformationMessage(
            `Repository '${newRepository.name}' created successfully!`,
            ...actions,
          );
