...
import {
  posNoRemote,
  preNoRemote,
  preRepositoryCreateNoRemote as preRepositoryCreationNoRemote,
} from './noRemote';
...
async function getWorkspaceFolderPathToPublish() {
  ...
  if (publishableFolders.length === 0) return undefined;
...
export async function uiPublish(): Promise<void> {
  try {
    if (!User.token) throw new Error("User token isn't set!");
    ...
    if (state === 'noGit') await preNoGit({ cwd });
    else await preNoRemote({ cwd });
        ...
        (async () => {
          ...
          if (action === actions[0]) {
            ...
            await vscode.commands.executeCommand(
              'vscode.open',
              vscode.Uri.parse(repositoryUrl),
            );
          }
        })(),
      ]); // End of Promise.all
