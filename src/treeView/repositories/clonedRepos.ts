...
export function activateClonedRepos(): void {
  ...
  commands.registerCommand(
    'githubRepoMgr.commands.clonedRepos.open',
    ({ repo }: RepoItem<true>) =>
      repo.localPath &&
  ...
  // Open in New Window
  commands.registerCommand(
    'githubRepoMgr.commands.clonedRepos.openInNewWindow',
    ({ repo }: RepoItem<true>) =>
  ...
  // Add to Workspace
  commands.registerCommand(
    'githubRepoMgr.commands.clonedRepos.addToWorkspace',
    ({ repo }: RepoItem<true>) =>
      workspace.updateWorkspaceFolders(workspace.workspaceFolders?.length ?? 0, 0, {
        uri: Uri.file(repo.localPath),
      }),
  );

  // Open Containing Folder
  commands.registerCommand(
    'githubRepoMgr.commands.clonedRepos.openContainingFolder',
    ({ repo }: RepoItem<true>) =>
      // revealFileInOS always open the parent path. So, to open the repo dir in fact, we pass the
      commands.executeCommand(
        'revealFileInOS',
        Uri.file(path.resolve(repo.localPath, '.git')),
      ),
  );

  // Copy local path to clipboard
  commands.registerCommand(
    'githubRepoMgr.commands.clonedRepos.copyPath',
    ({ repo }: RepoItem<true>) => {
  ...
  // Delete repo
  commands.registerCommand(
    'githubRepoMgr.commands.clonedRepos.delete',
    async ({ repo }: RepoItem<true>) => {
      if (!repo.localPath) return; // DO nothing if repo hasn't local path
      ...
      const answer = await window.showWarningMessage<MessageItem>(
        title,
        {
          detail: message,
          modal: true,
...
function sortClonedRepos<R extends Repository<true, 'user-is-member'>[]>(
  repos: R,
  userLogin: string,
): R {
  return repos.sort((a, b) => {
    // User repos comes first
    if (a.ownerLogin === userLogin && b.ownerLogin !== userLogin) return -1;
    if (a.ownerLogin !== userLogin && b.ownerLogin === userLogin) return 1;
...
export function getClonedTreeItem({
  repos,
  userLogin,
}: {
...
function sortOtherLocalRepos<
  R extends Repository<true, 'no-remote' | 'user-not-member'>[],
>(repos: R): R {
  return repos.sort((a, b) => {
...
// TODO: Add remember cloned repos when not logged option?
export function getClonedOthersTreeItem({
  repos,
}: {
