...
async function checkState(path: string): Promise<WorkspaceFolderState> {
  try {
    ...
    // gitHasRemote throws error if path has no git.
    return 'noGit';
  }
}
...
class WorkspaceClass {
  ...
  public get publishableFoldersAndStates(): {
    state: PublishableFolderState;
    path: string;
    name: string;
  }[] {
    return this.foldersAndStates.filter(
      (w) => w.state === 'gitWithoutRemote' || w.state === 'noGit',
    ) as any; // We are sure.
  }
  ...
  private updated() {
    const containsNoGit = this.workspaceFolderSpecial.find(
      (w) => w.state === 'noGit',
    );
    const containsGitWithoutRemote = this.workspaceFolderSpecial.find(
      (w) => w.state === 'gitWithoutRemote',
    );
    void myExtensionSetContext(
      'canPublish',
      containsNoGit || containsGitWithoutRemote,
    );
  }
  ...
  private async resetGitWatcher() {
    ...
    this.workspaceFolderSpecial = await Promise.all(
      workspaceFolders.map(async (workspaceFolder) => {
        const watcherDir = vscode.workspace.createFileSystemWatcher(
          `${workspaceFolder.uri.path}/.git`,
        ); // just watching config wouldn't catch ./git deletion.
        const watcherFile = vscode.workspace.createFileSystemWatcher(
          `${workspaceFolder.uri.path}/.git/config`,
        );
